...
import type { MetaCreate, MetaUpdate, Meta } from '../types'
...
export default function MetasPage() {
  ...
  const openModal = (item?: Meta) => {
    ...
    setForm(item ? { nombre:item.nombre, objetivo:String(item.monto_objetivo), actual:String(item.monto_actual), fechaObj:item.fecha_objetivo||'' }
            ...
            <div key={m.id} style={{ background:C.surface, border:`1px solid ${C.border}`, borderTop:`3px solid ${done?C.success:C.primary}`, borderRadius:14, padding:'1rem 1.25rem' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                <div>
                  ...
                  <p style={{ color:C.muted, fontSize:12, marginTop:2 }}>{done ? '✅ Completada' : `${m.dias_restantes} días restantes`}</p>
                </div>
                <span style={{ background:done?C.success+'22':C.primary2+'22', color:done?C.success:C.primary2, borderRadius:20, padding:'2px 10px', fontSize:11, fontWeight:600 }}>{done?'Completada':'Activa'}</span>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, background:'#1C1C1F', borderRadius:10, padding:'8px 0', marginBottom:12 }}>
                {[['Ahorrado',fmt(m.monto_actual),C.success],['Objetivo',fmt(m.monto_objetivo),'#fff'],['Restante',fmt(m.monto_restante),C.warn]].map(([l,v,c]) => (
        ...
        <div onClick={() => setModal(false)} style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,.8)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:C.surface, borderRadius:20, padding:'1.5rem', width:'100%', maxWidth:420 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              ...
              <div><label style={{ fontSize:11, color:C.muted, display:'block', marginBottom:3 }}>Monto ahorrado (S/)</label><input type="number" placeholder="0" value={form.actual} onChange={set('actual')} style={inputStyle}/></div>
